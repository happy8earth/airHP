...
────────────────────────────
등압 열교환기 모델 (Hot HX / Cold HX 공용).

  등압 과정: P = const
  Q_dot = m_dot * (h_out - h_in)   [W]
...
def run(state_in: ThermodynamicState,
        T_out: float,
        m_dot: float,
        label: str = "HeatExchanger") -> ComponentResult:
    """
    Parameters
    ----------
    ...
    ComponentResult
        ...
        W_dot     : 0.0
        Q_dot     : 열량 [W]  Hot HX: 음수 / Cold HX: 양수 (냉동 능력)
    """
