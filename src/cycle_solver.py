...
def _find_P_high(cycle_module, config: dict, use_run_cycle: bool) -> float:
    ...
    def error(P_high):
        if use_run_cycle:
            try:
                ...
                T_turb_out = out["T_turbine_outlet"]
            except ValueError:
            ...
            T_turb_out = results[2].state_out.T
        return T_turb_out - T_target
    ...
    if error(P_lo) * error(P_hi) > 0:
        raise ValueError(
            ...
            f"  T_turb at P_lo={error(P_lo)+T_target:.2f} K, "
            f"T_turb at P_hi={error(P_hi)+T_target:.2f} K"
        )
...
def solve(config: dict) -> dict:
    ...
    if use_run_cycle:
        ...
        T_turbine_outlet  = cycle_out["T_turbine_outlet"]
        Q_cold            = cycle_out["Q_cold"]
        W_compressor      = cycle_out["W_compressor"]
        W_turbine         = cycle_out["W_turbine"]
        ...
        states = [
            ...
            seq_results[2].state_out,   # Turbine 출구    → State 4
        ]
        component_results = seq_results
        T_turbine_outlet  = states[3].T
        Q_cold            = seq_results[3].Q_dot    # ColdHX, 양수
        W_compressor      = seq_results[0].W_dot    # 양수
        W_turbine         = -seq_results[2].W_dot   # 음수→양수
        Q_recuperator     = 0.0
    ...
    return dict(
        ...
        T_turbine_outlet = T_turbine_outlet,
        Q_cold           = Q_cold,
